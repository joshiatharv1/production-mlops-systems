...
// ─── Fake Edge Device Feed ────────────────────────────────────────────────────
const generateEdgeEvent = () => {
...
// ─── BBox Overlay Canvas ─────────────────────────────────────────────────────
function BBoxCanvas({ detections, width, height }) {
  const ref = useRef();
  useEffect(() => {
  ...
  return <canvas ref={ref} width={width} height={height} style={{ position:"absolute", top:0, left:0, pointerEvents:"none" }} />;
}
  ...
  return (
    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, display: "flex", flexDirection: "column", gap: 4, maxHeight: 200, overflowY: "auto" }}>
      {events.slice(0, 15).map((e, i) => (
        <div key={i} style={{
          display: "grid", gridTemplateColumns: "70px 80px 100px 1fr auto",
          ...
          <div style={{ background: "#1a2240", borderRadius: 3, overflow: "hidden", height: 14, alignSelf: "center" }}>
            <div style={{ width: `${e.confidence * 100}%`, height: "100%", background: MATERIAL_COLORS[e.material] + "99" }} />
          </div>
          <span style={{ color: "#667799" }}>{(e.confidence * 100).toFixed(0)}%</span>
        </div>
      ))}
    </div>
  );
...
export default function RecyclingDashboard() {
  ...
  const [edgeEvents, setEdgeEvents] = useState(() => Array.from({ length: 8 }, generateEdgeEvent));
  const [activeTab, setActiveTab] = useState("vision");
  const fileRef = useRef();

  // Simulate edge device stream
  useEffect(() => {
    const t = setInterval(() => {
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .tab-btn { background:none; border:none; cursor:pointer; padding:10px 20px; font-family:'Space Mono',monospace; font-size:12px; letter-spacing:.1em; text-transform:uppercase; border-radius:8px; transition:all .2s; }
        .tab-btn.active { background:rgba(0,229,255,.12); color:#00e5ff; }
        .tab-btn:not(.active) { color:#556688; }
        .tab-btn:hover:not(.active) { color:#8899cc; }
        .analyze-btn { border:none; cursor:pointer; padding:10px 24px; border-radius:8px; font-family:'Space Mono',monospace; font-size:12px; letter-spacing:.1em; text-transform:uppercase; transition:all .2s; font-weight:700; }
        .analyze-btn:hover { transform:translateY(-1px); filter:brightness(1.15); }
      `}</style>
      ...
      <div style={{ padding: "16px 32px 0", display: "flex", gap: 4 }}>
        {["vision","analytics","edge-feed"].map(t => (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 24 }}>
            ...
            <div className="card-glow" style={{
              ...
              <div style={{
                ...
                <button className="analyze-btn"
                  style={{ background: analyzing ? "#1a3060" : "linear-gradient(135deg,#00e5ff,#0080ff)", color: analyzing ? "#5577bb" : "#000" }}
                ...
                <div style={{ display: "flex", gap: 8, marginLeft: "auto" }}>
                  {SAMPLE_IMAGES.map((s, i) => (
                    <div key={i} onClick={() => { setImgSrc(s); setDetections([]); setAnalyzed(false); }}
                      style={{
                        width: 36, height: 36, borderRadius: 6, overflow: "hidden", cursor: "pointer",
                        border: imgSrc === s ? "2px solid #00e5ff" : "2px solid transparent", opacity: imgSrc === s ? 1 : 0.5,
                        transition: "all .2s",
                      }}>
              ...
              <div style={{ position: "relative", width: "100%", background: "#040914" }}>
                ...
                {analyzed && (
                  <BBoxCanvas detections={detections} width={480} height={440} />
                )}
              </div>

              {/* Status bar */}
              <div style={{ padding: "10px 18px", background: "rgba(0,0,0,0.3)", display: "flex", gap: 20, fontSize: 11, fontFamily: "'Space Mono', monospace", color: "#4466aa" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div className="card-glow" style={{
                ...
                <div style={{ fontSize: 11, fontFamily: "'Space Mono', monospace", color: "#4466aa", letterSpacing: "0.12em", marginBottom: 14 }}>DETECTION RESULTS</div>
                {!analyzed ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {detections.map((d, i) => (
                      <div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: `${MATERIAL_COLORS[d.label]}10`, border: `1px solid ${MATERIAL_COLORS[d.label]}33` }}>
              ...
              <StatCard label="Error Rate" value={errorRate} sub="Below 5% threshold ✓" accent="#06d6a0" />
        ...
        {activeTab === "analytics" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
              ...
              <StatCard label="Error Rate" value={errorRate} sub="Weekly avg" accent="#ff6b6b" />
            </div>
                ...
                {["EDGE-1","EDGE-2","EDGE-3","EDGE-4"].map((d, i) => (
                  <div key={d} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: i < 3 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: i < 3 ? "#06d6a0" : "#ff6b35" }}>{i < 3 ? "ONLINE" : "OFFLINE"}</span>
                    </div>
                  </div>
                ))}
              </div>
